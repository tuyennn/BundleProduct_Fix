...
varienGrid.prototype = {
    ...
    reload : function(url){
        ...
        // Begin the fix
        var substring = 'productss';

        if(url.indexOf(substring) !== -1) {
            var regex = new RegExp('/\\productss\/([^\/]+)\/');
            var productss = regex.exec(url)[1];
            var res = url.replace('/productss/' + productss, '');

            url = res;

            this.reloadParams.productss = productss;
